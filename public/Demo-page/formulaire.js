...
submitBtn.addEventListener("click", async (e) => {
  ...
  if(btnText.textContent!= "Aucun Produit Sélectionné"){
    ...
    const data1 = await loginFetch(nom.textContent,mail.textContent,phone.textContent,'.email-error',null,'auth')
    console.log(data1);
    const data3 = await loginFetch(nom.textContent,mail.textContent,phone.textContent,'.email-error',cleanSelectedProduct,'traker')
    console.log(data3)
    const data2 = await loginFetch(null,null,null,'.email-error',cleanSelectedProduct,'page')
    console.log(data2)
...
loginSubmit.addEventListener('click', async (e) => {
  ...
  if(loginName.value && loginMail.value){
    isError = await loginFetch(loginName.value,loginMail.value,null,'.login-email-error',null,'auth/login')
    if(!isError){
      errorShake(3)
      errorShake(4)
    }
